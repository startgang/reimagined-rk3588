...
- Интеграция с преобразователями интерфейсов (мостами):
    ...
    - LVDS: Для подключения LVDS-камер можно использовать мост Lontium LT2911R, который преобразует LVDS в MIPI CSI-2.
    - FPGA (преимущественно Gowin): FPGA выступает универсальным решением для приема любых нестандартных интерфейсов (включая высокоскоростные CameraLink). FPGA выполняет роль «умного» моста: декодирует входной сигнал и упаковывает его в пакеты MIPI/CSI или передает по PCIe.
